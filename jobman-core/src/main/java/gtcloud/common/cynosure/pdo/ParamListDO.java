...
public class ParamListDO extends ArrayList<ParamDO> implements Freezable, FreezerJSON
{
    private static final ParamDO NULL_ELEM = new ParamDO();

    public ParamListDO() {
        // EMPTY
    }

    public ParamListDO(Collection<ParamDO> collection) {
        super(collection);
    }

    public ParamListDO(int initialCapacity) {
        super(initialCapacity);
    }

    static private ParamDO deepCopyElem(ParamDO srcElem) {
        return (ParamDO)srcElem.makeClone();
    }

    // 从给定对象拷贝数据到当前对象
    @Override
    public void copyFrom(Freezable other) {
        ParamListDO from = (ParamListDO)other;
        this.clear();
        for (ParamDO src : from) {
            ParamDO e = deepCopyElem(src);
            this.add(e);
        }
    }
    ...
    public Freezable makeClone() {
        ParamListDO obj = new ParamListDO();
        obj.copyFrom(this);
        return obj;
    }
    ...
    public void defreeze(ByteStream input) throws DefreezeException {
        ...
        if (sz > 0) {
            ...
            for (int i = 0; i < sz; i++) {
                ParamDO elem = defreezeElem(input);
                this.add(elem);
            }
        }
    ...
    static private void freezeElem(ParamDO elem, ByteStream output) throws FreezeException {
        elem.freeze(output);
    }

    static private ParamDO defreezeElem(ByteStream input) throws DefreezeException {
        ParamDO elem = new ParamDO();
        elem.defreeze(input);
        return elem;
    }
