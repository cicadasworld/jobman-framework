...
public class FileListDO extends ArrayList<FileDO> implements Freezable, FreezerJSON
{
    private static final FileDO NULL_ELEM = new FileDO();

    public FileListDO() {
        // EMPTY
    }

    public FileListDO(Collection<FileDO> collection) {
        super(collection);
    }

    public FileListDO(int initialCapacity) {
        super(initialCapacity);
    }

    static private FileDO deepCopyElem(FileDO srcElem) {
        return (FileDO)srcElem.makeClone();
    }

    // 从给定对象拷贝数据到当前对象
    @Override
    public void copyFrom(Freezable other) {
        FileListDO from = (FileListDO)other;
        this.clear();
        for (FileDO src : from) {
            FileDO e = deepCopyElem(src);
            this.add(e);
        }
    }
    ...
    public void defreeze(ByteStream input) throws DefreezeException {
        ...
        if (sz > 0) {
            ...
            for (int i = 0; i < sz; i++) {
                FileDO elem = defreezeElem(input);
                this.add(elem);
            }
        }
    ...
    static private void freezeElem(FileDO elem, ByteStream output) throws FreezeException {
        elem.freeze(output);
    }

    static private FileDO defreezeElem(ByteStream input) throws DefreezeException {
        FileDO elem = new FileDO();
        elem.defreeze(input);
        return elem;
    }
