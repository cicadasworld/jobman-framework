...
public class SubjobDumpStateListDO extends ArrayList<SubjobDumpStateDO> implements Freezable, FreezerJSON
{
    private static final SubjobDumpStateDO NULL_ELEM = new SubjobDumpStateDO();

    public SubjobDumpStateListDO() {
        // EMPTY
    }

    public SubjobDumpStateListDO(Collection<SubjobDumpStateDO> collection) {
        super(collection);
    }

    public SubjobDumpStateListDO(int initialCapacity) {
        super(initialCapacity);
    }

    static private SubjobDumpStateDO deepCopyElem(SubjobDumpStateDO srcElem) {
        return (SubjobDumpStateDO)srcElem.makeClone();
    }

    // 从给定对象拷贝数据到当前对象
    @Override
    public void copyFrom(Freezable other) {
        SubjobDumpStateListDO from = (SubjobDumpStateListDO)other;
        this.clear();
        for (SubjobDumpStateDO src : from) {
            SubjobDumpStateDO e = deepCopyElem(src);
            this.add(e);
        }
    }
    ...
    public Freezable makeClone() {
        SubjobDumpStateListDO obj = new SubjobDumpStateListDO();
        obj.copyFrom(this);
        return obj;
    }
    ...
    public void freeze(ByteStream output) throws FreezeException {
        ...
        output.writeSize(sz);
        for (SubjobDumpStateDO elem : this) {
            if (elem == null) {
                elem = NULL_ELEM;
            }
    ...
    public void defreeze(ByteStream input) throws DefreezeException {
        ...
        if (sz > 0) {
            ...
            for (int i = 0; i < sz; i++) {
                SubjobDumpStateDO elem = defreezeElem(input);
                this.add(elem);
            }
        }
    ...
    static private void freezeElem(SubjobDumpStateDO elem, ByteStream output) throws FreezeException {
        elem.freeze(output);
    }

    static private SubjobDumpStateDO defreezeElem(ByteStream input) throws DefreezeException {
        SubjobDumpStateDO elem = new SubjobDumpStateDO();
        elem.defreeze(input);
        return elem;
    }
