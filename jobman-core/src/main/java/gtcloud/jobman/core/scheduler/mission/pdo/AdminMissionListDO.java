...
public class AdminMissionListDO extends ArrayList<AdminMissionDO> implements Freezable, FreezerJSON
{
    private static final AdminMissionDO NULL_ELEM = new AdminMissionDO();

    public AdminMissionListDO() {
        // EMPTY
    }

    public AdminMissionListDO(Collection<AdminMissionDO> collection) {
        super(collection);
    }

    public AdminMissionListDO(int initialCapacity) {
        super(initialCapacity);
    }

    static private AdminMissionDO deepCopyElem(AdminMissionDO srcElem) {
        return (AdminMissionDO)srcElem.makeClone();
    }

    // 从给定对象拷贝数据到当前对象
    @Override
    public void copyFrom(Freezable other) {
        AdminMissionListDO from = (AdminMissionListDO)other;
        this.clear();
        for (AdminMissionDO src : from) {
            AdminMissionDO e = deepCopyElem(src);
            this.add(e);
        }
    }
    ...
    public Freezable makeClone() {
        AdminMissionListDO obj = new AdminMissionListDO();
        obj.copyFrom(this);
        return obj;
    }
    ...
    public void defreeze(ByteStream input) throws DefreezeException {
        ...
        if (sz > 0) {
            ...
            for (int i = 0; i < sz; i++) {
                AdminMissionDO elem = defreezeElem(input);
                this.add(elem);
            }
        }
    ...
    static private void freezeElem(AdminMissionDO elem, ByteStream output) throws FreezeException {
        elem.freeze(output);
    }

    static private AdminMissionDO defreezeElem(ByteStream input) throws DefreezeException {
        AdminMissionDO elem = new AdminMissionDO();
        elem.defreeze(input);
        return elem;
    }
